...
def train(fps, args):
    ...
    z = tf.keras.Input(
        shape=(args.train_batch_size, args.wavegan_latent_dim),
        dtype=tf.float32,
    )
    # Make generator
    G_z = WaveGANGenerator(z, train=True, **args.wavegan_g_kwargs)
    if args.wavegan_genr_pp:
        G_z = tf.keras.layers.Conv1D(1,
                                     args.wavegan_genr_pp_len,
                                     use_bias=False,
                                     padding='same')(G_z)
    generator = tf.keras.Model(z, G_z)
    G_vars = tf.compat.v1.get_collection(
        tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope='G')
    ...
    writer = tf.summary.create_file_writer(cwd / "summary")
    ...
    G_z_rms = tf.sqrt(
        tf.reduce_mean(input_tensor=tf.square(G_z[:, :, 0]), axis=1))
    ...
    tf.compat.v1.summary.scalar('G_z_rms',
                                tf.reduce_mean(input_tensor=G_z_rms))
    """

    # Make real discriminator
    D_x = WaveGANDiscriminator(x, **args.wavegan_d_kwargs)
    D_vars = tf.compat.v1.get_collection(
        tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope='D')
    ...
    with tf.compat.v1.name_scope('D_G_z'), tf.compat.v1.variable_scope(
            'D', reuse=True):
    ...
    Q_vars = tf.compat.v1.get_collection(
        tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope='Q')
    ...
    if args.wavegan_loss == 'dcgan':
        ...
        G_loss = tf.reduce_mean(
            input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(logits=D_G_z,
                                                                 labels=real))
        ...
        D_loss = tf.reduce_mean(
            input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(logits=D_G_z,
                                                                 labels=fake))
        D_loss += tf.reduce_mean(
            input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(logits=D_x,
                                                                 labels=real))
        ...
        def q_cost_tf(z, q):
            ...
            lcat = tf.nn.softmax_cross_entropy_with_logits(
                labels=tf.stop_gradient(z_cat), logits=q_cat)
            return tf.reduce_mean(input_tensor=lcat)
        ...
        alpha = tf.random.uniform(shape=[args.train_batch_size, 1, 1],
                                  minval=0.,
                                  maxval=1.)
        differences = G_z - x
        interpolates = x + (alpha * differences)
        with tf.compat.v1.name_scope('D_interp'), tf.compat.v1.variable_scope(
                'D', reuse=True):
        ...
        slopes = tf.sqrt(
            tf.reduce_sum(input_tensor=tf.square(gradients), axis=[1, 2]))
    ...
    if args.wavegan_loss == 'dcgan':
        G_opt = tf.compat.v1.train.AdamOptimizer(learning_rate=2e-4, beta1=0.5)
        D_opt = tf.compat.v1.train.AdamOptimizer(learning_rate=2e-4, beta1=0.5)
    elif args.wavegan_loss == 'lsgan':
        G_opt = tf.compat.v1.train.RMSPropOptimizer(learning_rate=1e-4)
        D_opt = tf.compat.v1.train.RMSPropOptimizer(learning_rate=1e-4)
    elif args.wavegan_loss == 'wgan':
        G_opt = tf.compat.v1.train.RMSPropOptimizer(learning_rate=5e-5)
        D_opt = tf.compat.v1.train.RMSPropOptimizer(learning_rate=5e-5)
    elif args.wavegan_loss == 'wgan-gp':
        G_opt = tf.compat.v1.train.AdamOptimizer(learning_rate=1e-4,
                                                 beta1=0.5,
                                                 beta2=0.9)
        D_opt = tf.compat.v1.train.AdamOptimizer(learning_rate=1e-4,
                                                 beta1=0.5,
                                                 beta2=0.9)
        Q_opt = tf.compat.v1.train.RMSPropOptimizer(learning_rate=1e-4)
    else:
        raise NotImplementedError()
